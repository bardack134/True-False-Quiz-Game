class QuizBrain():
    #recibe un parametro
    def __init__(self, question_list):
        
        #Parametro de QuizBrain cuyo valor por defecto es "0"
        self.question_number=0
        
        #Parametro de QuizBrain que recibe un valor externo llamado question_list, question list sera nuestro "question_bnank" es decir lista de preguntas
        self.question_list = question_list
        
        self.score=0
    
    def still_has_question(self):
        
        length = len(self.question_list)
        
        if self.question_number==length:
            
            print("You have completed the quiz game")
            
            print(f"Your final score was: {self.score}/{length}")
            
            return False
        
        
        else:
            
            return True       
        
    
    def next_question(self ):
        
        current_question = self.question_list[self.question_number]
        
        self.question_number +=1
        
        #Le preguntamos al usuario, la respuesta de la pregunta actual
        user_answer=input(f"Q.{self.question_number}: {current_question.text} (True/False): " )
    
        self.check_answer(user_answer, correct_answer=current_question.answer)
        
            
    def check_answer(self, user_answer, correct_answer):
        
        if user_answer.lower()==correct_answer.lower():
            
            print("you got it right") 
            
            self.score += 1 
            
            
            
        else:
            print("That's wrong")
            
            print(f"The correct answer was {correct_answer}")
            
        print(f"Your current score is {self.score}/ curent question {self.question_number}")    
        print()    
            